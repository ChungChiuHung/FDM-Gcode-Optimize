...
def test_first_layer_bed_adhesion():
    ...
    state, temp = calculate_thermal_state(base_temp=220, island_extrude_dist=5.0, is_first_layer=True)
    assert state == "BED_ADHESION"
    assert temp == 220

def test_thermal_cooling_mode():
    """確保遇到微小特徵（擠出距離小於 15mm）且非首層時，會自動降溫 5 度"""
    state, temp = calculate_thermal_state(base_temp=220, island_extrude_dist=10.0, is_first_layer=False)
