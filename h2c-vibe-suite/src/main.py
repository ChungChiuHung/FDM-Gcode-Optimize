...
# 2. CRITICAL FIX: 把「專案根目錄 (Project Root)」加入系統路徑
# 這樣 Python 才能正確解析 'from src.core import ...'
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__)) # 這是 src/ 目錄
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)              # 這是專案最外層目錄
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# 3. Set up Centralized Logging (確保 logs 資料夾建立在專案最外層)
...
try:
    ...
    logger.error(f"CRITICAL: Failed to import pipeline.py")
...
def main(file_path):
    ...
    try:
        ...
        opt_success = pipeline.auto_optimize_gcode(file_path)
